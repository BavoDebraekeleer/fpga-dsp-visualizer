...
class CoetoolGui(QMainWindow):
    def __init__(self):
        ...
        self.fileName: str = ''
        self.out_type = 'RGB332'
        self.setOptionOutRGB332()
    ...
    def read(self):
        if self.fileName:
            self.setWindowTitle("coetool - " + self.fileName)
            self.statusBar().showMessage(self.fileName + ' loaded')

            if QFileInfo(self.fileName).suffix() == 'coe':

                self.conversion = CoeConverter(self.fileName)

                with open(self.fileName, encoding='utf-8') as coefile:
                ...
                self.generateTempCoe()

    def generateTempCoe(self):
        ...
        tmpcoe.close()
        self.conversion = CoeConverter(self.fileName)
        self.conversion.createCoe(self.out_type, tmpcoe.name)
    ...
    def saveCoe(self):
        ...
        # self.conversion.createCoe(out_type, coefilename+'.coe')  # Results in double '.coe' extension.
        self.conversion.createCoe(self.out_type, coefilename)
    ...
    def about(self):
        QMessageBox.about(self, "About coetool",
                ...
                "Greyscale option added by BavoD - August 2023")

    def setOptionOutRGB332(self):
        self.out_type = 'RGB332'
        self.rgb332Act.setEnabled(False)
        self.rgb332Act.setChecked(True)
        self.greyscaleAct.setEnabled(True)
        self.greyscaleAct.setChecked(False)
        self.read()

    def setOptionOutGreayscale(self):
        self.out_type = 'Greyscale'
        self.greyscaleAct.setEnabled(False)
        self.greyscaleAct.setChecked(True)
        self.rgb332Act.setEnabled(True)
        self.rgb332Act.setChecked(False)
        self.read()
    ...
    def createActions(self):
        ...
        self.rgb332Act = QAction("Output RGB332 HEX", self, enabled=True,
                checkable=True, triggered=self.setOptionOutRGB332)
        self.greyscaleAct = QAction("Output Greyscale Binary", self, enabled=True,
                checkable=True, triggered=self.setOptionOutGreayscale)
    ...
    def createMenus(self):
        ...
        self.optionsMenu.addAction(self.rgb332Act)
        self.optionsMenu.addAction(self.greyscaleAct)
