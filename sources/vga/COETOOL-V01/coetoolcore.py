...
class CoeConverter:
    ...
    def __init__(self, in_file):
        ...
        # RGB palette generation: Red(3bits) Green(3bits) Blue(2bits)/1pixel = 256 colors
        # (as used in .coe VGA mem files). NECESSARY
    ...
    def createCoe(self, out_type: str, out_file):
        with open(out_file, encoding='utf-8', mode='wt') as out_coe_file:
            if out_type == 'Greyscale':
                out_coe_file.write('; VGA Memory Map\n; .COE file with binary coefficients\n; Greyscale\n; Height: ' + str(self.height) + ', Width: ' + str(self.width) + '\n; Depth: 3-bit\n; Pixels: ' + str(int(self.height) * int(self.width)) + '\n\nmemory_initialization_radix=2;\nmemory_initialization_vector=\n')
                for i, b in enumerate(self.imgbytes):
                    # For greyscale images
                    bi = str("{0:08b}".format(int(b, 16)))
                    r = int(bi[:3], 2)
                    g = int(bi[3:6], 2)
                    b = int(bi[6:] + "1", 2)
                    me = int((r + b + g) / 3)
                    me_bi = str("{0:03b}".format(me))
                    out_coe_file.write(me_bi + ',\n')
            else:
                out_coe_file.write('; VGA Memory Map\n; .COE file with hex coefficients\n; RGB332\n; Height: ' + str(self.height) + ', Width: ' + str(self.width) + '\n; Depth: 16-bit\n; Pixels: ' + str(int(self.height) * int(self.width)) + '\n\nmemory_initialization_radix=16;\nmemory_initialization_vector=\n')
                for i, b in enumerate(self.imgbytes):
                    # Original RGB332
                    if i > 0 and i % 16 == 0:  # TO-DO check if necessary, check in FPGA -> Nope!
                        out_coe_file.write('\n')
